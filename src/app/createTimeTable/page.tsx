...
const Grid: React.FC = () => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(true); // Control the modal
  const [userName, setUserName] = useState('');
  const [timetableId, setTimetableId] = useState<string | null>(null); // Store the timetable ID
  ...
  const handleSubmitUser = async () => {
    if (userName.trim()) {
      try {
        // API call to create the new timetable entry
        const response = await axios.post('/api/qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqtimetable', { name: userName });
        setTimetableId(response.data.id); // Set the returned ID from the backend
        setIsModalOpen(false); // Close the modal once the user submits their name
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-black text-black">
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full text-center">
            <h2 className="text-2xl mb-4 text-gray-900">Enter your name</h2>
            <input
              type="text"
              value={userName}
              onChange={(e) => setUserName(e.target.value)}
              placeholder="Your Name"
              className="w-full p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-600 mb-4 text-black"
            />
            <button
              onClick={handleSubmitUser}
              className="bg-purple-600 text-white px-6 py-2 rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-700"
            >
              Submit
            </button>
          </div>
      ...
      <div className={`container mx-auto px-4 py-8 text-white relative ${isModalOpen ? 'blur-md' : ''}`}>
